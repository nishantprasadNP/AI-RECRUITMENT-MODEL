    ...
    def _clean_text(self, text: str) -> str:
        ...
        for i, line in enumerate(cleaned_lines):
            ...
            if not line:
                ...
                if prev_line:
                    # Case 1: Previous line is a section heading or ends in a colon
                    ...
                    if next_line:
                        ...
                        # Case 2: Short consecutive lines (list items, labels/values)
                        prev_words = prev_line.split()
                        next_words = next_line.split()
                        if len(prev_words) <= 3 and len(next_words) <= 3:
                            continue

                        # Case 3: Next line starts with a list bullet
                        if next_line.startswith(("-", "*", "•", "▪", "◦", "▪")) or (
                            next_words and next_words[0].endswith(".") and next_words[0][:-1].isdigit()
                        ):
                            continue
