...
class JobExtractor:
    ...
    def extract(self, job_text: str) -> JobProfile:
        ...
        # Guard: detect when a JSON blob (e.g. a resume or profile JSON) is passed
        # instead of a real job description.  The LLM returns all-empty fields in
        # this case because JSON data has no JD-language for the prompt to extract from.
        stripped = job_text.strip()
        if stripped.startswith(("{", "[")):
            import json as _json
            try:
                _json.loads(stripped)
                # If it parsed cleanly it IS a JSON object/array — not a JD.
                logger.error(
                    "Job extraction failed: input appears to be a JSON object/array, not a "
                    "job description. Did you accidentally pass a resume profile or extracted "
                    "JSON instead of raw JD text?"
                )
                raise EmptyJobTextError(
                    "The input text looks like a JSON blob, not a job description. "
                    "Please pass the raw job description text (e.g. contents of a .txt file), "
                    "not a structured JSON object."
                )
            except (_json.JSONDecodeError, ValueError):
                # Not valid JSON — fine to proceed
                pass
